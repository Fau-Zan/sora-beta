...
const argSession = getArgValue('--session') || getArgValue('-s')
const sessionName = argSession || process.env.SESSION_NAME || 'zan'
const usePairFlag = argv.includes('--pair') || argv.includes('-p') || process.env.USE_PAIR === 'true'
...
export async function autoStart(
  ...
  const store = await createPostgresStore({ namespace: configName, connectionString: POSTGRES_URL })

  const client = new BaseClient()
  client.sock = MainStart(config)
  client.store = store as WhatsType.IPostgresStore
...
autoStart(sessionName, usePairFlag)